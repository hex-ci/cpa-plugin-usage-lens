...
对标 [cpa-usage-keeper](https://github.com/Willxup/cpa-usage-keeper)，用量分析功能面照抄、前端用 Vue 3 + TS 重写，
但同时**砍掉了独立服务的一切负担**：不单起进程、不占额外端口、不碰 Redis、不要任何密钥。

## 差异化：零独立服务

| | 独立服务（Keeper） | usage-lens |
|---|---|---|
| 部署 | 独立进程 + 端口 + 常驻守护 | 放一个动态库 + 配置开一个开关，随 CPA 启停 |
| 采集 | Redis 队列订阅/轮询 | 事件推送（同源、非破坏、零延迟） |
...
| 共存迁移 | 独占队列，多实例冲突 | 非破坏性采集，可与 Keeper 并存灰度 |
...
- 事件为广播复制，不消费 Redis 队列，与独立 Keeper 共存互不干扰。
...
3. 重启 CPA。面板入口：`http://<cpa>/v0/resource/plugins/usage-lens/panel`（主面板左侧菜单「用量透镜」）。
...
## 已裁剪（相对 cpa-usage-keeper）

凭证页（auth files / AI 提供商 / 配额）、社区排行、独立登录、多语言、版本检查、定时备份、预聚合与 90 天归档。
